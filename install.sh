...
if [ -f "$INSTALL_DIR/requirements.txt" ]; then
    "$PY_BIN" -m pip install --quiet -r "$INSTALL_DIR/requirements.txt" > /dev/null 2>&1
else
    "$PY_BIN" -m pip install --quiet flask requests werkzeug opencv-python-headless xknx soco zeroconf > /dev/null 2>&1
fi
...
# 4. Creazione del servizio Systemd (per l'avvio automatico con il lanciatore protetto)
